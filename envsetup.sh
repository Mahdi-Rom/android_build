...
function addcompletions()
{
    ...
    dirs="sdk/bash_completion vendor/mahdi/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
...
function print_lunch_menu()
{
    ...
    echo
    if [ "z${MAHDI_DEVICES_ONLY}" != "z" ]; then
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the MAHDI github
        T=$(gettop)
        pushd $T > /dev/null
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var MAHDI_VERSION)
        ZIPFILE=mahdi-$MODVERSION.zip
        ZIPPATH=$OUT/$ZIPFILE
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
    ...
    if (adb shell cat /system/build.prop | grep -q "ro.mahdi.device=$MAHDI_BUILD");
    then
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $MAHDI_BUILD, run away!"
    fi
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $MAHDI_FIXUP_COMMON_OUT ]; then
